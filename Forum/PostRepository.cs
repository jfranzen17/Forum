...
namespace Forum
{
    ...
    class PostRepository
    {
        ...
        {
            using var connection = new SqliteConnection(_connectionString);
            var sql = "SELECT * FROM posts AS p " + " INNER JOIN Users AS u ON (u.UserId = p.UserId) WHERE p.ThreadId = @ThreadId;";
        ...
        {
            using var connection = new SqliteConnection(_connectionString);
            var sql = "INSERT INTO posts (Text,UserId,ThreadId) VALUES (@Text, @UserId, @ThreadId)";
